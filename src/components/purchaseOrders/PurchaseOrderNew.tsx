...
export default function PurchaseOrderNew() {
  ...
  const addProductRow = (product?: ProductRow) => {
    const newProduct: ProductRow = product || {
      id: Date.now().toString(),
      sku: "",
      productName: "",
      quantity: 0,
  ...
  const handleImport = async () => {
    ...
    if (result.success && result.data) {
      ...
      messageApi.success("Import file thành công! Vui lòng xác nhận để thêm vào danh sách.");
    } else {
      messageApi.error(result.message || "Import file thất bại!");
    }
  };
